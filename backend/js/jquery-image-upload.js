$(document).ready(function () {
    ...
    /*****************************
     Variables
     *****************************/
    var imgWidth = jQuery("#upload_width").val(),
        imgHeight = jQuery("#upload_height").val(),
        zindex = 0;
    	dropzone = $('#droparea'),
        uploadBtn = $('#uploadbtn'),
	...
	imgWidth = jQuery("#upload_width").val();
	imgHeight = jQuery("#upload_height").val();
    ...
    var readFile = function (file) {
		...
		ext = allowed_extensions.indexOf(ext);
        if (ext>-1) {
			// change media type
    ...
    /*****************************
     Draw Image Preview
     *****************************/
    var createPreview = function (file,success) {
        //populate jQuery Template binding object
		var randvalue = Math.ceil(Math.random()*25);
		var randsign = Math.ceil(Math.random()*2);
		if(randsign==2)randvalue = -randvalue;
		var re = /(?:\.([^.]+))?$/;
		var ext = re.exec(file.name)[1];
		if(success){
			fail_upload = "";
			...
			_msg = file_not_uploaded;
		}
		
		var _icon = "";
		switch(ext){
			case"pdf":
				_icon = '<img src="'+file_icons_base+'/pdf.png" />';
				break;
			case"doc":case"docx":
				_icon = '<img src="'+file_icons_base+'/word.png" />';
				break;
			case"xls":case"xlst":case"xlsx":
				_icon = '<img src="'+file_icons_base+'/xsl.gif" />';
				break;
			case"jpg":case"jpeg":case"png":case"bmp":case"gif":
				_icon = '<img src="'+file_icons_base+'/img.png" />';
				break;
			case"rar":case"gz":case"bz":case"tar.gz":case"tar.bz":case"zip":case"iso":
				_icon = '<img src="'+file_icons_base+'/compressed.jpg"  />';
				break;
			default:
				_icon = "<i class='fa fa-file-text-o' style='font-size:40px;'></i>";
		}
        var img = $("<div class='imageholder "+fail_upload+"'>"+_icon+"<br />"+_msg+"<span class='ext'>"+ext+"<span></div>").prependTo("#result")
            .hide().css({
    ...
    var uploadToServer = function (oldFile) {
        ...
        formData.append('upload_file', oldFile);
		formData.append('csrf_minetoken_name',$.cookie('csrf_minecookie_name'));
		formData.append('file_name',jQuery('#upload').val().replace(/^.*[\\\/]/, ''));
        //submit formData using $.ajax
        $.ajax({
            url: site_url+'qajax/research_upload',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
            ...
            success: function (data) {
                if(data=="0")createPreview(oldFile,false);
				else{
					createPreview(oldFile,true);
					$("#research_form").append('<input type="hidden" value="'+data+'" name="research_file[]" />');
				}
				$("#is_uploading").fadeOut();
            }
        });
    }
